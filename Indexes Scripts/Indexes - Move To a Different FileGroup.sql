IF OBJECT_ID('MoveIndexToFileGroup') IS NOT NULL
    DROP PROC [dbo].[MoveIndexToFileGroup];
GO
...
BEGIN
    ...
    BEGIN
	   ...
	   + QUOTENAME(@DBName) + '.sys.Objects so INNER JOIN '
	   + QUOTENAME(@DBName)
	   + '.sys.Indexes si ON so.Object_Id = si.Object_id INNER JOIN '
	   + QUOTENAME(@DBName)
	   + '.sys.FileGroups sf ON sf.Data_Space_Id = si.Data_Space_Id INNER JOIN '
	   + QUOTENAME(@DBName)
	   + '.sys.Index_columns sic ON si.Object_Id = sic.Object_Id AND si.Index_id = sic.Index_id INNER JOIN '
	   + QUOTENAME(@DBName)
	   + '.sys.Columns sc ON sic.Column_Id = sc.Column_Id and sc.Object_Id = sic.Object_Id '
	   + ' WHERE so.Name = ''' + @ObjectName + ''''
	   + ' AND so.Schema_id = '
	   + CAST(SCHEMA_ID(@SchemaName) AS VARCHAR(25))
	   ...
	   BEGIN
		  ...
		  SELECT
			 @IndexKeySQL = (
				CASE WHEN @IndexKeySQL = '' THEN 
				    ( 
					   @IndexKeySQL + QUOTENAME(ColumnName) + CASE WHEN Is_Descending_Key = 0 THEN ' ASC' ELSE ' DESC' END 
				    )
				ELSE 
				    ( 
					   @IndexKeySQL + ',' + QUOTENAME(ColumnName) + CASE WHEN Is_Descending_Key = 0 THEN ' ASC' ELSE ' DESC' END 
		  ...
		  SELECT
			 @IncludeColSQL = (
				CASE WHEN @IncludeColSQL = '' THEN 
				    ( @IncludeColSQL + QUOTENAME(ColumnName) )
				ELSE 
				    ( @IncludeColSQL + ',' + QUOTENAME(ColumnName) )
				END
			 )
		  ...
		  SELECT
			 @FinalSQL = 'CREATE ' + @IsUnique + @Type
			 + ' INDEX ' + QUOTENAME(@IndName) + ' ON '
			 + QUOTENAME(@DBName) + '.'
			 + QUOTENAME(@SchemaName) + '.'
			 + QUOTENAME(@ObjectName) + '(' + @IndexKeySQL
			 + ') '
			 + CASE WHEN LEN(@IncludeColSQL) <> 0
			 ...
			 + 'FILLFACTOR = '
			 + CAST(@FillFactor AS VARCHAR(3)) + ') ON '
