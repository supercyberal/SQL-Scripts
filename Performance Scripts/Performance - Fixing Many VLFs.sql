...
--
-- 2014-03-29 - ACOSTA - Changed the log backup call to use OLA LOG backup.
--
-- 2014-05-16 - ACOSTA - Adding an option to compress backup based on server setting "backup compression"
--
-- 2015-01-21 - ACOSTA - Added DBCC LOGINFO to each DB for VLF check.
--
-- 2015-02-05 - ACOSTA - Added parameter @ChangeBackupType to DatabaseBackup backup proc in order to take a DIFF/FULL backup if a tran log cant be performed.
--
-- 2015-05-06 - ACOSTA - Added FULL backup statement for non-SIMPLE recovery DBs after breaking the log chain.
...
-- ACOSTA - 2014-03-29 = Using the default backup folder for the LOG job.
...
SELECT
	@bckpath = [SettingValue]
FROM @tBackupFolder;

-- ACOSTA - 2014-05-16 = Adding an option to compress backup based on server setting "backup compression"
DECLARE @cCompression CHAR(1);
SET @cCompression = 'N';

IF EXISTS (SELECT 1 FROM sys.[configurations] WHERE [name] LIKE 'Backup Comp%')
	SET @cCompression = 'Y';
...
BEGIN
	...
	BEGIN
		...
		--PRINT 'USE master;' + CHAR(13) + 'IF EXISTS (SELECT 1 FROM sys.[backup_devices] WHERE [name] = ''BckLog'') EXEC master.dbo.sp_dropdevice @logicalname = N''BckLog''' + CHAR(13) + 'EXEC sp_addumpdevice ''disk'', ''BckLog'', ''' + @bckpath + '\example_bck.trn'';'

		-- ACOSTA - 2014-03-29 = Adding OLA LOG job.
		--PRINT '-- Backup Log'
		PRINT 'EXECUTE master.[dbo].[DatabaseBackup] @Databases = ''' + @dbname + ''', @Directory = N''' + @bckpath + ''', @BackupType = ''LOG'', @Verify = ''N'', @CheckSum = ''Y'', @Compress=''' + @cCompression + ''', @ChangeBackupType = ''Y'', @LogToTable = ''Y'';'
		--PRINT '--'
	...
	-- Add FULL backup after resizing the LOG for non-SIMPLE recovery DBs.
	IF @recmodel <> 'SIMPLE'	   
	   PRINT 'EXECUTE master.[dbo].[DatabaseBackup] @Databases = ''' + @dbname + ''', @Directory = N''' + @bckpath + ''', @BackupType = ''FULL'', @Verify = ''N'', @CheckSum = ''Y'', @Compress=''' + @cCompression + ''', @LogToTable = ''Y'';'
