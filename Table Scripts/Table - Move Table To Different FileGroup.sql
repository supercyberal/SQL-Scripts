...
11/10/2010  Alvaro Costa		  Transformed script into a SP.	   1.10		  
==================================================================================*/

IF OBJECT_ID('MoveTableToDiffFileGroup') IS NOT NULL
    DROP PROC [dbo].[MoveTableToDiffFileGroup];
GO

CREATE PROC [dbo].[MoveTableToDiffFileGroup]
(
    @TableName SYSNAME
	, @TableSchemaName SYSNAME
    , @SourceFileGroup SYSNAME
    , @TargetFileGroup SYSNAME
...
BEGIN
	...
	DECLARE @TableSchemaToMove NVARCHAR(128);
	DECLARE @TableSchemaWholeToMove NVARCHAR(128);
    ...
    -- Source FileGroup
    SELECT
	   @SourceFileGroupID = [data_space_id]
    FROM sys.[filegroups]
    WHERE [name] = @SourceFileGroup;

    -- Destination FileGroup
    SELECT
	   @TargetFileGroupID = [data_space_id]
    FROM sys.[filegroups]
    WHERE [name] = @TargetFileGroup;

    SET @TableToMove = @TableName;
	SET @TableSchemaToMove = @TableSchemaName;
	SET @TableSchemaWholeToMove = (@TableSchemaToMove + '.' + @TableToMove);
    ...
    IF (SELECT TOP 1 groupid FROM sysindexes WHERE id = OBJECT_ID(@TableSchemaWholeToMove) and indid IN (0, 1)) <> @SourceFileGroupID
    BEGIN
	   SET @ScriptMsg = N'The table ' + @TableSchemaWholeToMove + ' is not found on filegroup ' + CAST(@SourceFileGroupID AS VARCHAR(32)) + N'. Please provide a valid table name and source file group.';
	   RAISERROR(@ScriptMsg, 16, 1);
	   RETURN;
    END
    ...
    SET @TableHasCI = OBJECTPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), 'TableHasClustIndex');
    ...
    IF @TableHasCI = 0
    BEGIN
	   SET @TableHasIdent = OBJECTPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), 'TableHasIdentity');

	   IF @TableHasIdent = 0
	   BEGIN
		  SET @TableHasPK = OBJECTPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), 'TableHasPrimaryKey');
		  SET @TableHasUQ = OBJECTPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), 'TableHasUniqueCnst');
		  ...
		  BEGIN
			 EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] ADD [This_Is_My_Ident_Col_Name] BIGINT IDENTITY (1, 1) ');
			 ...
			 ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N']([' + @IdentColName + '])
			 ON [' + @File2Name + N']');

			 -- The table is now moved -> Remove the CI.
			 EXEC(N'DROP INDEX [' + @TableSchemaToMove + N'].[' + @TableToMove + N'].[This_Is_My_Clsuetered_Index_Name]');

			 -- Finally, remove the added identity column
			 EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] DROP COLUMN [This_Is_My_Ident_Col_Name] ');
		  END
		  ELSE
		  BEGIN
			 ...
			 SELECT  
				COL_NAME(OBJECT_ID(@TableSchemaWholeToMove), colid)
				-- append the DESC/ASC string, based on the ASC/DESC order of the PK columns
				, (				
				    CASE WHEN INDEXKEY_PROPERTY(OBJECT_ID(@TableSchemaWholeToMove), INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), @KeyName, 'IndexID'), keyno,'IsDescending') = 1 THEN 
					   'DESC'
				    ELSE 
					   'ASC'
				    END
			 ...
			 WHERE id = OBJECT_ID(@TableSchemaWholeToMove)
			 AND indid = INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), @KeyName, 'IndexID');
			 ...
			 WHILE EXISTS (SELECT 1 FROM #tblColTable WHERE Idx = @i)
			 BEGIN
			 ...
			 EXEC(N'CREATE CLUSTERED INDEX [This_Is_My_Clsuetered_Index_Name]
			 ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](' + @ColList + ')
			 ON [' + @File2Name + N']');
	   ...
	   BEGIN
		  ...
		  SELECT 
			 @IdentColName = COLUMN_NAME
		  FROM INFORMATION_SCHEMA.COLUMNS WITH (NOLOCK)
		  WHERE TABLE_NAME = @TableToMove
		  AND COLUMNPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), COLUMN_NAME, 'IsIdentity') = 1;
    ...
    BEGIN
	   ...
	   SELECT 
		  ...
		  -- append the DESC/ASC string, based on the ASC/DESC order of the PK columns
		  , (			 
			 CASE WHEN INDEXKEY_PROPERTY(OBJECT_ID(@TableSchemaWholeToMove), INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), @CIName, 'IndexID'), keyno, 'IsDescending') = 1 THEN 
				'DESC'
			 ELSE 
				'ASC'
			 END
	   ...
	   SELECT @i = MIN(Idx) FROM #tblColTable;
	   SET @ColList = N'';
	   ...
	   IF (
		  OBJECTPROPERTY(OBJECT_ID(@CIName), 'IsPrimaryKey') = 1
		  OR OBJECTPROPERTY(OBJECT_ID(@CIName), 'IsUniqueCnst') = 1
		  OR INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), @CIName, 'IsUnique') = 1
	   )
	   BEGIN
		  ...
		  WHERE rkeyid = OBJECT_ID(@TableSchemaWholeToMove)
		  AND rkeyindid = INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), @CIName, 'IndexID');

		  SELECT @AssocFKeyName = MIN(KeyName) FROM #tblFKTable;
		  ...
		  BEGIN
			 ...
			 SELECT @j = MIN(Idx) FROM #tblFKColTable;
			 ...
			 SET @SQLStr = LEFT(@SQLStr, LEN(@SQLStr) - 1) + N') REFERENCES [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](';
		  ...
		  BEGIN
			 EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] DROP CONSTRAINT [' + @CIName + N'] ');
			 EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] WITH NOCHECK ADD CONSTRAINT [' + @CIName + N'] PRIMARY KEY CLUSTERED (' + @ColList + N') ON [' + @File2Name + N']');
		  END
		  ELSE
		  BEGIN
			 ...
			 BEGIN
				EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] DROP CONSTRAINT [' + @CIName + N'] ');
				EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] WITH NOCHECK ADD CONSTRAINT [' + @CIName + N'] UNIQUE CLUSTERED (' + @ColList + N') ON [' + @File2Name + N']');
			 END
			 ELSE -- the CI here is a unique index
			 BEGIN
				SET @IsPadIndex = INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), @CIName, 'IsPadIndex');

				EXEC(N'DROP INDEX [' + @TableSchemaToMove + N'].[' + @TableToMove + N'].[' + @CIName + N']');
				...
				IF @IsPadIndex = 1
				    EXEC(N'CREATE UNIQUE CLUSTERED INDEX [' + @CIName + N'] ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](' + @ColList + N') WITH PAD_INDEX ON [' + @File2Name + N']');
				ELSE
				    EXEC(N'CREATE UNIQUE CLUSTERED INDEX [' + @CIName + N'] ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](' + @ColList + N') ON [' + @File2Name + N']');
			 END
		  END
	   ...
	   BEGIN
		  ...
		  IF @IsPadIndex = 1
			 EXEC(N'CREATE CLUSTERED INDEX [' + @CIName + N'] ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](' + @ColList + N') WITH PAD_INDEX ON [' + @File2Name + N']');
		  ELSE
			 EXEC(N'CREATE CLUSTERED INDEX [' + @CIName + N'] ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](' + @ColList + N') ON [' + @File2Name + N']');
	   END
    END
    ...
    BEGIN
	   ...
	   INNER JOIN sysobjects b WITH (NOLOCK)
		  ON a.[name] = b.[name]
	   WHERE b.parent_obj = OBJECT_ID(@TableSchemaWholeToMove)
    ...
    BEGIN
	   ...
	   SELECT  
		  ...
		  , a.indid
		  , (
			 CASE WHEN INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), a.[name], 'IsUnique') = 1 THEN 
				'UI' -- to denote unique index.
			 ELSE 
				'I'  -- to denote a non-unique index
			 END
		  )
	   FROM sysindexes a WITH (NOLOCK)
	   LEFT OUTER JOIN #tblKeysTable b
	   ON a.name = b.KeyName
	   WHERE   a.id = OBJECT_ID(@TableSchemaWholeToMove)
	   AND INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), a.[name], 'IsStatistics') = 0
	   AND a.groupid = @SourceFileGroupID
    ...
    BEGIN
	   ...
	   BEGIN
		  ...
		  FROM sysreferences WITH (NOLOCK)
		  WHERE rkeyid = OBJECT_ID(@TableSchemaWholeToMove)
		  AND rkeyindid = INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), @KeyName, 'IndexID');
		  ...
		  IF @Type = 'PK'
		  BEGIN
			 EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] DROP CONSTRAINT [' + @KeyName + N'] ');
			 EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] WITH NOCHECK ADD CONSTRAINT [' + @KeyName + N'] PRIMARY KEY NONCLUSTERED (' + @ColList + N') ON [' + @File2Name + N']');
		  END
		  ELSE
		  BEGIN
			 IF @Type = 'UQ'
			 BEGIN
				EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] DROP CONSTRAINT [' + @KeyName + N'] ');
				EXEC(N' ALTER TABLE [' + @TableSchemaToMove + N'].[' + @TableToMove + N'] WITH NOCHECK ADD CONSTRAINT [' + @KeyName + N'] UNIQUE NONCLUSTERED (' + @ColList + N') ON [' + @File2Name + N']');
			 END
			 ELSE -- must be UI
			 BEGIN
				SET @IsPadIndex = INDEXPROPERTY(OBJECT_ID(@TableSchemaWholeToMove), @KeyName, 'IsPadIndex');

				EXEC(N' DROP INDEX [' + @TableSchemaToMove + N'].[' + @TableToMove + N'].[' + @KeyName + N']');

				IF @IsPadIndex = 1
				    EXEC(N' CREATE UNIQUE NONCLUSTERED INDEX [' + @KeyName + N'] ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](' + @ColList + N') WITH PAD_INDEX ON [' + @File2Name + N']');
				ELSE
				    EXEC(N' CREATE UNIQUE NONCLUSTERED INDEX [' + @KeyName + N'] ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](' + @ColList + N') ON [' + @File2Name + N']');
			 END
		  END
	   ...
	   BEGIN
		  ...
		  IF @IsPadIndex = 1
			 EXEC(N' CREATE NONCLUSTERED INDEX [' + @KeyName + N'] ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](' + @ColList + N') WITH PAD_INDEX ON [' + @File2Name + N']');
		  ELSE
			 EXEC(N' CREATE NONCLUSTERED INDEX [' + @KeyName + N'] ON [' + @TableSchemaToMove + N'].[' + @TableToMove + N'](' + @ColList + N') ON [' + @File2Name + N']');
	   END
