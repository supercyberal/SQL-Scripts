...
IF OBJECT_ID('ScriptCreateTableKeys') IS NOT NULL
...
BEGIN
    ...
    SET @sql = '' +
        'SELECT ' +
            'ISNULL(CASE ' +
                'WHEN 1 IN (i.is_primary_key, i.is_unique) THEN ' +
                    '''ALTER TABLE '' + ' +
                ...
                'WHEN 1 IN (i.is_unique_constraint) THEN ' +
        ...
        'WHERE ' +
            ...
            CASE @version
				...
				WHEN '2012' THEN 'AND i.has_filter = 0 '
                ELSE ''
            END +
...
IF OBJECT_ID('ScriptDropTableKeys') IS NOT NULL
...
BEGIN
    ...
    SET @sql = '' +
        'SELECT ' +
            'ISNULL(statement,'''') AS [-- Drop Candidate Keys] ' +
        'FROM ' +
        '( ' +
            'SELECT ' +
                'CASE ' +
