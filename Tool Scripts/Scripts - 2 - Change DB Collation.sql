...
------------------------------------------------------------------------------------------------------------------------------------------------
-- 0. Define which collation to apply (RUN THIS IN TEXT MODE).

-- Check server collation
SELECT CAST(SERVERPROPERTY('Collation') AS sysname)

IF OBJECT_ID('TempDB..#NewCollation') IS NOT NULL
	DROP TABLE #NewCollation;

CREATE TABLE #NewCollation (
	ID INT IDENTITY(1,1) NOT NULL
	, NewCollation sysname NOT NULL
);

INSERT [#NewCollation] ([NewCollation])
VALUES  ('');
...
SELECT @CollationName = [NewCollation] FROM [#NewCollation];
...
SET @SQLStmt = @SQLStmt + CHAR(13) + 'SELECT * FROM sys.[databases]';
